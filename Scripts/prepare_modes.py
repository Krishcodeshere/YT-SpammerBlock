...
def prepare_filter_mode_smart(scanMode, config, miscData, sensitive=False):
  ...
  if config['filter_mode'] == "autosmart":
    ...
    if sensitive:
      ...
      print("\n----------------------------------------------- Smart Mode -----------------------------------------------")
    print(f"~~~ This mode automatically scans for multiple spammer techniques ~~~\n")
    ...
    # if sensitive == False:
    #   print(f" > {F.LIGHTRED_EX}NOTE:{S.R} This prioritizes a {F.LIGHTGREEN_EX}VERY low false positive rate{S.R}, at the cost of occasionally missing some spammers.\n")
    # # elif sensitive == True:
      # print(f" > {F.LIGHTRED_EX}NOTE:{S.R} In sensitive mode, {F.LIGHTRED_EX}expect more false positives{S.R}. Recommended to run this AFTER regular Smart Mode.\n")
  ...
  while validInput == False:
    ...
    try:
        ...
        if countChoice > 0 and countChoice <= quotaLimit:
          ...
          print(f"\n{F.LIGHTRED_EX}Error:{S.R} {countChoice} is too many comments, you'll run out of API Quota. ")
        else:
